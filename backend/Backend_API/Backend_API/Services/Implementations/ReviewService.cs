    ...
    public class ReviewService : IReviewService
    {
        ...
        public async Task<ReviewResponseDto> CreateReviewAsync(long reviewerId, long listingId, ReviewCreateDto dto)
        {
            ...
            bool isVerifiedTenant = await _context.Rentals
                .AnyAsync(r => r.ListingId == listingId && r.TenantId == reviewerId && r.Status == "ended");
        ...
        public async Task<List<ReviewResponseDto>> GetReviewsByListingAsync(long listingId, string? type = null, string? sortBy = null, long? currentUserId = null)
        {
            await EnsureRentalsAndUpgradeReviewsTableAsync();
            ...
            if (!string.IsNullOrWhiteSpace(sortBy))
            {
                ...
                {
                    case "popular":
                        query = query.OrderByDescending(r => r.ReviewLikes.Count).ThenByDescending(r => r.CreatedAt);
                        break;
                    case "highest_rating":
            ...
            var rentedListingUserIds = await _context.Rentals
                .Where(r => r.ListingId == listingId && r.Status == "ended")
                .Select(r => r.TenantId)
                .ToListAsync();
        ...
        public async Task<List<ReviewResponseDto>> GetMyReviewsAsync(long reviewerId)
        {
            ...
            var userRentedListings = await _context.Rentals
                .Where(r => r.TenantId == reviewerId && r.Status == "ended")
        ...
        public async Task<ReviewResponseDto> UpdateReviewAsync(long reviewerId, long reviewId, ReviewUpdateDto dto)
        {
            ...
            // Re-detect spam
            bool isVerifiedTenant = await _context.Rentals
                .AnyAsync(r => r.ListingId == review.ListingId && r.TenantId == reviewerId && r.Status == "ended");
        ...
        private async Task<ReviewResponseDto?> BuildResponseDto(long reviewId, long? currentUserId = null)
        {
            ...
            bool isVerifiedTenant = await _context.Rentals
                .AnyAsync(r => r.ListingId == review.ListingId && r.TenantId == review.ReviewerId && r.Status == "ended");

            return MapToDto(review, isVerifiedTenant, currentUserId);
        }

        public async Task<(double AverageRating, int ReviewCount)> GetListingRatingAsync(long listingId)
        {
        ...
        {
            var reviews = await _context.Reviews
                .Where(r => r.ListingId == listingId && r.Type == "review" && r.Status == "approved" && !r.IsDeleted)
                .Select(r => r.Rating)
                .ToListAsync();
