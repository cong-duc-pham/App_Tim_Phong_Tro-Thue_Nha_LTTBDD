...
namespace Backend_API.Services.Implementations
{
    public class ReviewService : IReviewService
    {
        ...
        public ReviewService(PhongTroDbContext context, INotificationService notificationService)
        {
            _context = context;
        ...
        )
        {
            ...
            var listing =
                await _context.Listings.FirstOrDefaultAsync(l => l.ListingId == listingId)
                ?? throw new Exception("Không tìm thấy tin đăng.");

            bool isVerifiedTenant = await _context.Rentals.AnyAsync(r =>
            ...
            // Recalculate listing rating cache

            await RecalculateListingRatingsAsync(listingId);
            await RecalculateLandlordReputationAsync(listing.LandlordId);

            var result =
                await BuildResponseDto(review.ReviewId)
        ...
        public async Task<List<ReviewResponseDto>> GetReviewsByListingAsync(
            long listingId,
            string? type = null,
            string? sortBy = null,
        ...
        )
        {
            ...
            var query = _context
                ...
                .Where(r => r.ListingId == listingId && r.Status == "approved" && !r.IsDeleted);
            ...
            {
                string normSort = sortBy.ToLower();
                ...
                switch (normSort)
                {
                    case "popular":
                        ...
                        query = query
                            .OrderByDescending(r => r.ReviewLikes.Count)
            ...
            var rentedListingUserIds = await _context
                .Rentals.Where(r =>
                    r.ListingId == listingId && (r.Status == "active" || r.Status == "ended")
                )
                .Select(r => r.TenantId)
                .ToListAsync();
            ...
            return reviews
                .Select(r =>
                    MapToDto(r, rentedListingUserIds.Contains(r.ReviewerId), currentUserId)
                )
                .ToList();
        }
        ...
        public async Task<List<ReviewResponseDto>> GetMyReviewsAsync(long reviewerId)
        {
            ...
            var userRentedListings = await _context
                .Rentals.Where(r =>
                    r.TenantId == reviewerId && (r.Status == "active" || r.Status == "ended")
                )
                .Select(r => r.ListingId)
                .ToListAsync();

            return reviews
                .Select(r => MapToDto(r, userRentedListings.Contains(r.ListingId), reviewerId))
                .ToList();
        }
        ...
        )
        {
            ...
            if (
                ...
                && (DateTime.UtcNow - review.CreatedAt.Value).TotalMinutes > 30
            )
            ...
            await RecalculateListingRatingsAsync(review.ListingId);
            await RecalculateLandlordReputationAsync(review.Listing.LandlordId);
        ...
        public async Task DeleteReviewAsync(long reviewerId, long reviewId)
        {
            var review =
                await _context
                    .Reviews.Include(r => r.ReviewImages)
                    .Include(r => r.ReviewLikes)
                    .FirstOrDefaultAsync(r => r.ReviewId == reviewId)
                ?? throw new Exception("Không tìm thấy đánh giá.");
            ...
            long listingId = review.ListingId;
        ...
        public async Task<ReviewResponseDto> ReplyReviewAsync(
            long landlordId,
            long reviewId,
            ReviewReplyDto dto
        )
        {
            ...
            var result =
                await BuildResponseDto(reviewId) ?? throw new Exception("Phản hồi thất bại.");
            ...
            {
                Console.WriteLine(
                    $"[Review] Could not notify tenant for reply {review.ReviewId}: {ex.Message}"
                );
            }
        ...
        public async Task<ReviewResponseDto> ToggleLikeAsync(long userId, long reviewId)
        {
            ...
            var existing = await _context.ReviewLikes.FirstOrDefaultAsync(x =>
                x.ReviewId == reviewId && x.UserId == userId
            );
            ...
            {
                await _context.ReviewLikes.AddAsync(
                    new ReviewLike
                    {
                        ReviewId = reviewId,
        ...
        private async Task<ReviewResponseDto?> BuildResponseDto(
            long reviewId,
            long? currentUserId = null
        )
        {
        ...
        public async Task<(double AverageRating, int ReviewCount)> GetListingRatingAsync(
            long listingId
        )
        {
            var listing = await _context.Listings.FirstOrDefaultAsync(l =>
            ...
            return (listing.AverageRating, listing.ReviewCount);
        }

        private async Task RecalculateListingRatingsAsync(long listingId)
        {
            var reviews = await _context
                .Reviews.Where(r =>
                    r.ListingId == listingId
            ...
            if (listing != null)
            {
                ...
                listing.TotalRatingSum = reviews.Sum(r => r ?? 0);

                listing.AverageRating =
                    reviews.Count > 0
                        ? Math.Round(listing.TotalRatingSum / (double)reviews.Count, 1)
                        : 0.0;
        ...
        private async Task RecalculateLandlordReputationAsync(long landlordId)
        {
            var ratings = await _context
                .Reviews.Include(r => r.Listing)
                .Where(r =>
                    r.Listing.LandlordId == landlordId
                    && r.Type == "review"
                    && r.Status == "approved"
                    && !r.IsDeleted
                    && r.Rating != null
            ...
            var landlord = await _context.Users.FirstOrDefaultAsync(u =>
                u.UserId == landlordId
            );
            if (landlord != null)
            {
                landlord.ReputationCount = ratings.Count;
                landlord.ReputationScore =
                    ratings.Count > 0 ? Math.Round(ratings.Average(r => r ?? 0.0), 1) : 5.0;
                await _context.SaveChangesAsync();
            }
        }

        private async Task NotifyLandlordReviewCreatedAsync(
            Listing listing,
        ...
        )
        {
            try
            {
                ...
                string title =
                    review.Type == "review"
                        ? "Đánh giá mới trên tin đăng"
                        : "Câu hỏi mới trên tin đăng";
            ...
            {
                Console.WriteLine(
                    $"[Review] Could not notify landlord for review {review.ReviewId}: {ex.Message}"
                );
            }
        }
        ...
        private static ReviewResponseDto MapToDto(
            ...
            new()
            {
                ...
                IsLiked =
                    ...
                    && r.ReviewLikes.Any(x => x.UserId == currentUserId.Value),
