    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task SendEmailVerificationOtpAsync(string toEmail, string otpCode)
        {
            ...
            if (string.IsNullOrWhiteSpace(host)
                ...
                || senderPassword.Contains("your-app-password", StringComparison.OrdinalIgnoreCase))
            {
                _logger.LogWarning("SMTP is not configured. Email verification OTP for {Email}: {OtpCode}", toEmail, otpCode);
                Console.WriteLine($"[DEV OTP] {toEmail}: {otpCode}");
                return;
            }
            ...
            message.From.Add(new MailboxAddress(senderName, senderEmail));
            message.To.Add(MailboxAddress.Parse(toEmail));
            message.Subject = "Ma OTP xac minh Email Swings House";
            message.Body = new TextPart("html")
            {
                Text = $@"
                    <div style=""font-family:Arial,sans-serif;line-height:1.6;color:#111827"">
                        <h2>Swings House</h2>
                        <p>Ma OTP xac minh Email cua ban la:</p>
                        <div style=""font-size:28px;font-weight:700;letter-spacing:6px;color:#0d6efd"">{otpCode}</div>
            ...
            {
                _logger.LogWarning(ex, "Could not send email verification OTP email in development. OTP for {Email}: {OtpCode}", toEmail, otpCode);
                Console.WriteLine($"[DEV OTP] {toEmail}: {otpCode}");
            }
        }
    }
